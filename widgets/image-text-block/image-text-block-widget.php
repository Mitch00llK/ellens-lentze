<?php
namespace EllensLentze\Widgets;

use \Elementor\Widget_Base;

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly.
}

/**
 * Image Text Block Widget.
 *
 * Elementor widget that displays an image and content side-by-side with switchable layout.
 *
 * @since 1.0.0
 */
class Image_Text_Block_Widget extends Widget_Base {

	public function get_name() {
		return 'image_text_block';
	}

	public function get_title() {
		return esc_html__( 'Image Text Block', 'ellens-lentze' );
	}

	public function get_icon() {
		return 'eicon-image-box';
	}

	public function get_categories() {
		return [ 'ellens-lentze' ];
	}

	public function get_keywords() {
		return [ 'image', 'text', 'block', 'cta', 'side' ];
	}

    public function get_style_depends() {
        return [ 'ellens-image-text-block' ];
    }

	protected function register_controls() {
		require_once __DIR__ . '/includes/controls/content-controls.php';
        \EllensLentze\Widgets\Image_Text_Block\Includes\Controls\Content_Controls::register( $this );

		require_once __DIR__ . '/includes/controls/style-controls.php';
        \EllensLentze\Widgets\Image_Text_Block\Includes\Controls\Style_Controls::register( $this );
	}

	protected function render() {
		require_once __DIR__ . '/includes/render/render-functions.php';
        \EllensLentze\Widgets\Image_Text_Block\Includes\Render\Render_Functions::render_widget( $this );
	}
}
